...
class Activos_equipos {
	
	public $id;
	public $nombre_equipo;
	public $familia;
	public $tiempoBaseOperacion;
	...
	public static function getAll() {
		...
		$sql = 'SELECT * FROM activos_equipos ORDER BY id DESC';
	...
	public static function getAllByEQuipo($equipo) {
		...
		// Build database query
		$sql = "SELECT * FROM activos_equipos WHERE nombre_equipo='$equipo' ";
	...
	public static function getAllEquiposFamilia($familia) {

		// Build database query
		$sql = "SELECT * FROM activos_equipos WHERE familia='$familia' ORDER BY nombre_equipo ASC";
	...
	public static function getAllInnerEamOrderAsc() 
	{

		// Build database query
		$sql = 'SELECT activos_equipos.*, activos_eam.descripcion, activos_eam.departamento 
				FROM activos_equipos 
				INNER JOIN activos_eam ON activos_equipos.nombre_equipo = activos_eam.activo
				ORDER BY activos_equipos.familia ASC, activos_equipos.nombre_equipo ASC';
		
		// Return objects
		return self::getBySql($sql);
	}

	public static function getByEamActivo($equipo) 
	{
		...
		$sql = "SELECT activos_equipos.*, activos_eam.descripcion, activos_eam.departamento 
				FROM activos_equipos 
				INNER JOIN activos_eam ON activos_equipos.nombre_equipo = activos_eam.activo
				WHERE activos_equipos.nombre_equipo = '$equipo' ";
		//echo $sql;
		// Return objects
		return self::getBySql($sql);
	}
	...
	public static function getAllTiempoBaseEquipo($equipo) {
	
		// Initialize result array
		$result = array();
		
		// Build database query
		$sql = "select nombre_equipo, tiempoBaseOperacion from activos_equipos where nombre_equipo = ?";
		...
		if ($statement->prepare($sql)) {
			...
			// Bind variable to prepared statement
			$statement->bind_result($nombre_equipo, $tiempoBaseOperacion);
		...
		// Build new object
		$object = new self;
		$object->nombre_equipo = $nombre_equipo;
		$object->tiempoBaseOperacion = $tiempoBaseOperacion;
		return $object;
	}
	...
	public function insert() {
		...
		// Build database query
		$sql = "insert into activos_equipos (nombre_equipo, familia, tiempoBaseOperacion) values (?, ?, ?)";
		...
		if ($statement->prepare($sql)) {
			...
			// Bind parameters
			$statement->bind_param('ssi', $this->nombre_equipo, $this->familia, $this->tiempoBaseOperacion);
